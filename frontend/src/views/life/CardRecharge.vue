  ...
  <div v-loading="loading">
    <div class="bg-white rounded-xl border border-soft p-5 mb-4">
      ...
      <div class="flex gap-3">
        <button v-for="a in amounts" :key="a" @click="doRecharge(a)" :disabled="sending" class="px-6 py-2 bg-ink text-white rounded-lg text-sm font-medium hover:bg-steel transition-colors">¥{{ a }}</button>
      </div>
    </div>
...
async function fetch() { loading.value = true; try { const r = await getRecharges({ page: page.value, size: 10 }); records.value = r.data.records; total.value = r.data.total; if (r.data.records.length && page.value === 1) balance.value = r.data.records[0].balance } finally { loading.value = false } }
async function doRecharge(amount: number) { sending.value = true; try { await recharge({ amount }); ElMessage.success('充值成功'); page.value = 1; fetch() } finally { sending.value = false } }
onMounted(fetch)
</script>

  ...
  <div>
    <div class="bg-white rounded-xl border border-soft p-5 mb-4">
      ...
      <div class="flex gap-3 items-end flex-wrap">
        ...
        <el-select v-model="form.targetType" size="small" class="!w-28"><el-option label="全体" value="all" /><el-option label="学生" value="student" /><el-option label="教师" value="teacher" /></el-select>
    ...
    <div class="bg-white rounded-xl border border-soft p-5">
      ...
      <div v-for="a in announcements" :key="a.id" class="flex justify-between py-2 border-b border-wash last:border-0 text-sm">
        ...
        <div class="text-xs text-mist">{{ a.sendTime?.substring(0,16) }} · {{ a.targetType==='all'?'全体':a.targetType==='student'?'学生':'教师' }}</div>
      </div>

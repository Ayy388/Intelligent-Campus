...
describe('userStore', () => {
    ...
    it('login should set token and role, then fetch user info', async () => {
        const mockLoginRes = { data: { token: 'test-token', role: 'admin' } };
        const mockUserRes = { data: { id: 1, username: 'admin', roleName: '管理员' } };
    ...
    it('fetchUserInfo should set userInfo from API', async () => {
        const mockUserRes = { data: { id: 1, username: 'student', roleName: '学生' } };
        vi.mocked(getCurrentUser).mockResolvedValue(mockUserRes);
        const store = useUserStore();
